...
public class BookController {
  ...
  public ResponseEntity<Book> getBooks(@PathVariable("id") Long id, @RequestParam(name = "error", defaultValue = "false") boolean error
          , @RequestParam(name = "retry", defaultValue = "false") boolean retry) throws BookException {
    ...
    Optional<Book> book = !error ? Optional.ofNullable(restClient.getBooksFromMockServer(false).getBody()) : retry ? Optional.ofNullable(restClient.getBooksFromMockServerWithRetry(true).getBody()) : Optional.ofNullable(restClient.getBooksFromMockServer(true).getBody());
